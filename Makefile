...
run:
	...
	@qemu-system-x86_64 -drive file=output/ElectroOS.img -m 256M -cpu qemu64 -drive if=pflash,format=raw,unit=0,file="OVMFbin/OVMF_CODE-pure-efi.fd",readonly=on -drive if=pflash,format=raw,unit=1,file="OVMFbin/OVMF_VARS-pure-efi.fd" -net none
clean:
	@if ! [ "$(shell id -u)" = 0 ];then\
		echo "Please run this script as root!";\
		exit 1;\
